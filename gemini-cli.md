...
After setting up your backend environment, install the Gemini CLI globally (or in your virtual environment):
...
## Starting the Backend and Gemini CLI

1. **Start the Gemini CLI in a new terminal:**
   ```powershell
   gemini
   ```
   (Leave this running. It will listen for commands.)

2. **Start the FastAPI backend as usual:**
   ```powershell
   uvicorn main:app --reload
   ```

Now both the backend and Gemini CLI will be running and ready for use.